...
import type { Detection, SpectrogramResponse } from '@/lib/types';
import { buildColormapLUT } from '@/lib/colormap';
import { formatFreqShort, formatTime } from '@/lib/format';
...
export function Waterfall({
  ...
  useEffect(() => {
    ...
    if (spectrogram && spectrogram.magnitude_2d.length > 0) {
      ...
      for (let t = 0; t < timeBins; t++) {
        const y = PAD_TOP + (t / timeBins) * plotHeight;
        const yNext = PAD_TOP + ((t + 1) / timeBins) * plotHeight;
        const rowHeight = Math.ceil(yNext - y);

        for (let f = 0; f < freqBins; f++) {
          const freq = spectrogram.freq_bins[f];
          if (freq < freqMin || freq > freqMax) continue;
          const x = freqToX(freq);
          const xNext = freqToX(spectrogram.freq_bins[Math.min(f + 1, freqBins - 1)]);
