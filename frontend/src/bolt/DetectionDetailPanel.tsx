...
export function DetectionDetailPanel({ detection, sampleRate, jobId }: DetectionDetailPanelProps) {
  ...
  const rows: Row[] = [
    ...
    { key: 'Center (Refined)', value: measurement(detection.center_frequency_refined_hz, 'Hz'),
      explain: 'Sharper of the M=2 (BPSK) / M=4 (QPSK) raised-tone peaks on the Mth-power spectrum, divided by M. Only attempted for constant-envelope candidates.' },
    ...
    { key: 'Fine modulation', value: detection.fine_modulation_label ?? 'Not reliably estimated',
      explain: 'Published only when the Mth-power phase-cluster circular spread is below 0.8 rad. Pulsed candidates are excluded (their gated carrier has trivially perfect phase concentration).' },
    { key: 'Symbol rate', value: detection.symbol_rate_hz == null ? (detection.symbol_rate_status == null || detection.symbol_rate_status.includes('not attempted') ? 'Not attempted' : 'Not reliably estimated') : measurement(detection.symbol_rate_hz, 'Hz'),
      explain: 'Lowest spectral peak within 3 dB of the differentiated-and-squared segment’s global maximum, excluding the first 4 DC-leakage bins. Requires a confirmed PSK fine label and SNR ≥ 4.5 dB.' },
